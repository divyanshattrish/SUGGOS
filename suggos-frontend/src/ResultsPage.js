...
/* ─────────────────────────────────────────
   ROOM PREVIEW — shows uploaded photo or fallback SVG
───────────────────────────────────────── */
    ...
    <div style={s.previewWrap}>
      {imageDataUrl ? (
        ...
        <img
          ...
          style={{ width: '100%', display: 'block', borderRadius: 16, objectFit: 'cover', maxHeight: 280 }}
        />
      ) : (
      ...
      {/* Active item label (only shown with SVG) */}
      {!imageDataUrl && activeId && (
        <div style={s.previewActiveLabel}>
          <span style={{ color: 'var(--rose)', marginRight: '0.4rem' }}>●</span>
          {suggestions.find(item => item.id === activeId)?.name}
        </div>
      )}

      {/* Your room badge (shown when real photo) */}
      {imageDataUrl && (
        <div style={s.previewActiveLabel}>
          <span style={{ color: 'var(--rose)', marginRight: '0.4rem' }}>📷</span>
          Your room
...
export default function ResultsPage() {
  ...
  // Get uploaded image from navigation state
  const imageDataUrl = location.state?.imageDataUrl || null;

  // Re-upload from results page
  ...
  const categories = ['All', ...Array.from(new Set(suggestions.map(item => item.category)))];
  const filtered = filter === 'All' ? suggestions : suggestions.filter(item => item.category === filter);
    ...
    <div style={s.page}>
      ...
      <main style={s.main}>
        ...
        <aside style={s.sidebar}>
          ...
          <button onClick={() => fileRef.current.click()} style={s.reuploadBtn}>
            ↑ Upload a new room
          </button>
        </aside>
...
const s = {
  ...
  main: { display: 'grid', gridTemplateColumns: '420px 1fr', minHeight: '100vh', paddingTop: 64 },
  sidebar: {
  ...
  insightRow: { display: 'flex', alignItems: 'flex-start', gap: '0.8rem', marginBottom: '0.9rem' },
  ...
  reuploadBtn: {
    ...
    cursor: 'pointer', marginTop: 'auto', fontFamily: 'var(--sans)',
  },
  suggestionsCol: { padding: '3rem 3rem 4rem', background: 'var(--ivory)', overflowY: 'auto' },
  suggestionsHeader: {
  ...
  budgetPill: {
    ...
    fontSize: '0.82rem', color: 'var(--stone)', whiteSpace: 'nowrap', alignSelf: 'flex-start',
  },
  ...
  filterTab: {
    ...
    fontSize: '0.8rem', color: 'var(--stone)', cursor: 'pointer', fontFamily: 'var(--sans)',
  },
  filterTabActive: { background: 'var(--dark)', border: '1px solid var(--dark)', color: '#fff' },
  cardsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.2rem' },
  card: {
    background: '#fff', borderRadius: 16, overflow: 'hidden', border: '1px solid #E8E0D8',
  ...
  cardThumb: { height: 140, position: 'relative' },
  cardTag: {
  ...
  cardReason: { fontSize: '0.8rem', color: 'var(--stone)', lineHeight: 1.65, fontWeight: 300, marginBottom: '1rem' },
  cardFooter: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  cardPrice: { fontFamily: 'var(--serif)', fontSize: '1.15rem', fontWeight: 500, color: 'var(--dark)' },
  cardRetailer: { fontSize: '0.7rem', color: 'var(--stone-light)', marginTop: '0.1rem' },
  cardBtn: {
    background: 'var(--rose)', color: '#fff', border: 'none', borderRadius: 100,
    padding: '0.5rem 1rem', fontSize: '0.78rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--sans)',
  },
};
